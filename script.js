...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Função para carregar os animais do banco de dados
  function carregarAnimais() {
      $.ajax({
          ...
          success: function(response) {
              if (response.success) {
                  const animais = response.animais;
...
/*function mostrarAnimais(animais) {
      ...
      animalList.empty(); // Limpar a lista antes de preenchê-la novamente

      animais.forEach(animal => {
          const animalDiv = $(`
              <div class="animal">
                  <img src="${animal.imagemanimal}" alt="${animal.nomeanimal}">
                  <p class="animal-name">${animal.nomeanimal}</p>
                  <p>${animal.idadeanimal}</p>
                  <p>${animal.detalheanimal}</p>
                  <p>${animal.porteanimal}</p>
                  <p>${animal.onganimal}</p>
                  <button class="detalhes-btn">Ver Detalhes</button>
              </div>
          `);

          animalDiv.find(".detalhes-btn").on("click", function() {
              const animalName = animal.nomeanimal;
              window.location.href = `animais.html?name=${encodeURIComponent(animalName)}`;
          });

          animalList.append(animalDiv);
      });
  }*/
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function fetchAnimals() {
        $.ajax({
            url: 'listar_animais.php', // URL do arquivo PHP que retorna a lista de animais
            type: 'GET',
            dataType: 'json',
...
const genderPara = document.createElement("p");
genderPara.textContent = animal.generoanimal;

const agePara = document.createElement("p");
agePara.textContent = animal.idadeanimal;

const breedPara = document.createElement("p");
breedPara.textContent = animal.racaanimal;

const ongPara = document.createElement("p");
ongPara.textContent = animal.onganimal;
...
animalCard.appendChild(breedPara);
